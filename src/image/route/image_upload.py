...
@image_upload.route("/user/<user_id>/all_image", methods=["GET"])
@token_required
def get_all_image(user_id):
    ...
    image_data = ImagePost.query.filter(ImagePost.uploaded_by == user_id).paginate(
        page=page, per_page=per_page
    )
    return jsonify(image_serializer(image_data)), status.HTTP_200_OK


@image_upload.route("/get/user_image/<image_id>", methods=["GET"])
@token_required
def get_image(image_id):
    ...
    image_data = ImagePost.query.get(image_id)
    return (
        jsonify(
            {
                "id": image_data.id,
...
@image_upload.route("/del/user_image/<id>", methods=["DELETE"])
@token_required
def update_image(id):
    """delete image"""
