...
@send_message.route("/send/message_user/<user_id>", methods=["POST"])
@token_required
def message_send(user_id):
    """send message"""
    ...
    send_messages = Message(
        message=messages, sender_id=g.user_data.id, message_receive=user_id
    )
...
@send_message.route("/post/send_user/<user_id>", methods=["POST"])
@token_required
def post_send(user_id):
    ...
    send_messages = Message(
        message=file_name, sender_id=g.user_data.id, message_receive=user_id
    )
    db.session.add(send_messages)
    db.session.commit()
...
@send_message.route("/del/user_message/<message_id>", methods=["DELETE"])
@token_required
def del_message(id):
    """delete message"""
