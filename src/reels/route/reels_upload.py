...
def upload_reel():
    ...
    extension = old_file_name.split(".")
    _uuid = uuid.uuid4()
    file_name = str(_uuid) + "." + extension[-1]
...
@reels_upload.route("/user/<user_id>/all_reels/", methods=["GET"])
@token_required
def get_all_reels(user_id):
    """get all reels"""
    ...
    reel_data = ReelsPost.query.filter(ReelsPost.uploaded_by == user_id).paginate(
...
@reels_upload.route("/get/user_reel/<reel_id>", methods=["GET"])
@token_required
def get_reel(reel_id):
    ...
    reels_data = ReelsPost.query.get(reel_id)
...
@reels_upload.route("/del/user_reel/<reel_id>", methods=["DELETE"])
@token_required
def del_reel(reel_id):
    """delete reel"""
